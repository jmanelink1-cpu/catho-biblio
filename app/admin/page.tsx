...
export default async function AdminDashboard() {
  ...
  const kpis = [
    { icon: '📚', label: 'Livres',            value: (totalBooks ?? 0).toLocaleString('fr-FR'), bg: 'var(--color-brand-soft)', fg: 'var(--color-brand)' },
    { icon: '👤', label: 'Utilisateurs',      value: totalUsers.toLocaleString('fr-FR'),        bg: '#EEF6FF', fg: '#2563EB' },
    { icon: '✅', label: 'Accès actifs',       value: activeUsers.toLocaleString('fr-FR'),        bg: '#F0FDF4', fg: '#16A34A' },
    { icon: '💰', label: 'Revenus (FCFA)',     value: revenue.toLocaleString('fr-FR'),            bg: 'var(--color-gold-soft)', fg: 'var(--color-gold)' },
  ]

  const mini = [
    { label: "Inscrits aujourd'hui", value: signupsToday },
    { label: 'Inscrits (7 jours)',   value: signupsWeek },
    { label: 'Inscrits (ce mois)',   value: signupsMonth },
    { label: 'Paiements en attente', value: pendingPays },
  ]
    ...
    <div>
      <div className="h-14 flex items-center justify-between px-6 border-b"
        ...
        <div className="flex gap-2">
          <a href="/library" className="text-xs font-semibold px-3 py-2 rounded-lg" style={{ background: 'var(--color-brand)', color: '#fff' }}>Ma bibliothèque →</a>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {kpis.map(s => (
            <div key={s.label} className="rounded-2xl border p-5 flex items-start gap-4"
                 style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
              <div className="w-11 h-11 rounded-xl flex items-center justify-center text-xl flex-shrink-0" style={{ background: s.bg, color: s.fg }}>{s.icon}</div>
              <div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {mini.map(m => (
            <div key={m.label} className="rounded-2xl border p-4" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
              <div className="text-xl font-extrabold" style={{ fontFamily: 'var(--font-sora)', color: 'var(--color-ink)' }}>{m.value}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 rounded-2xl border p-6" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            ...
            <div className="flex items-end gap-1" style={{ height: 140 }}>
              {daily.map((d, i) => (
                <div key={i} className="flex-1 flex flex-col justify-end items-center group" title={`${d.label} : ${d.count}`}>
                  <div style={{ width: '100%', height: `${(d.count / dailyMax) * 100}%`, minHeight: d.count > 0 ? 4 : 1,
                    background: d.count > 0 ? 'var(--color-brand)' : 'var(--color-border)', borderRadius: '3px 3px 0 0', transition: 'height .2s' }} />
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="rounded-2xl border p-6" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            ...
            <div className="space-y-2">
              {[
                ['/admin/books',    '📚', 'Gérer les livres'],
                ['/admin/users',    '👤', 'Gérer les utilisateurs'],
                ['/admin/payments', '💳', 'Voir les paiements'],
